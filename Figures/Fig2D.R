...
df_z <- read.csv(file = "../Data/SexSpecific_Genes_4wk_8wk_z-score.csv",stringsAsFactors = TRUE)
df_list <- read.xlsx("../Data/SI_Table1.xlsx",sheet = "Table S1.2")
#vec_filtered <- filter(df_list,BroadPattern=="Neonatal")$symbol # for neonatal genes 
vec_filtered <- filter(df_list,BroadPattern=="Pubertal")$symbol # for pubertal genes
...
### plot heatmap ###
tiff(file="Heatmap_SexGene_Timeseries_Puberty.tiff", units="in", width=5, height=5, res=150)
